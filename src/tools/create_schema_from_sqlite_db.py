...
if __name__ == '__main__':
    ...
    for f, df in db_files:
        ...
        prev_tab_num = len(ex_tabs[df]["table_names"]) if df in ex_tabs else 0
        prev_col_num = len(ex_tabs[df]["column_names"]) if df in ex_tabs else 0
        cur_tab_num = len(table["table_names"])
        cur_col_num = len(table["column_names"])
        if df in ex_tabs.keys() and prev_tab_num == cur_tab_num and prev_col_num == cur_col_num and prev_tab_num != 0 and len(
                ex_tabs[df]["column_names"]) > 1:
